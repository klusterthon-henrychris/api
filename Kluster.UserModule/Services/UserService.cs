    ...
    UserModuleDbContext context) : IUserService
{
    public async Task<ErrorOr<UserResponse>> GetLoggedInUser()
    {
        ...
        if (user is null)
        {
            logger.LogInformation("User not found for ID: {0}", userId);
            return Errors.User.NotFound;
        }

        logger.LogInformation("Retrieved logged-in user: {0}", user);
    ...
    {
        var userId = currentUser.UserId ?? throw new UserNotSetException();
        logger.LogInformation("Received request to update user from {0}.\nRequest: {1}", userId, request);
        ...
        logger.LogInformation("User updated: {0}", user.Id);
    ...
    public async Task<string> GenerateOtpForEmail(string userId)
    {
        ...
        logger.LogInformation("Generated OTP for user: {0}", user.Id);
    ...
    public async Task<ErrorOr<Success>> ConfirmEmailWithToken(string emailAddress, string otp)
    {
        ...
        if (user is null)
        {
            logger.LogInformation("User not found for email: {0}", emailAddress);
            return Errors.User.NotFound;
        }
        ...
        if (result.Succeeded)
        {
            logger.LogInformation("Email confirmed for user: {0}", user.Id);
        ...
        var errors = result.Errors
            ...
            .ToList();
        logger.LogInformation("Email confirmation failed for user: {0}", user.Id);
        return errors;
    }
    ...
    public async Task<ErrorOr<Success>> ResendVerificationMessage(string id, string verificationRoute)
    {
        ...
        {
            logger.LogInformation("Resending verification message to user: {0} (Email: {1})", user.Id, user.Email);
            await bus.Publish(new EmailOtpRequestedEvent(user.FirstName, user.LastName, user.Email!, user.Id));
            return Result.Success;
        }

        logger.LogInformation("SMS Validation not available for user: {0}", user.Id);
    ...
    public async Task<ErrorOr<Success>> SendForgotPasswordMail(ForgotPasswordRequest request)
    {
        ...
        if (user is null)
        {
            ...
            logger.LogInformation("User not found for email: {0}", request.EmailAddress);
            return Result.Success;
        }

        logger.LogInformation("Sending forgot password email to user: {0} (Email: {1})", user.Id, user.Email);
    ...
    public async Task<ErrorOr<Success>> ResetPassword(ResetPasswordRequest request)
    {
        ...
        logger.LogInformation("Resetting password for user: {0} (Email: {1})", user.Id, user.Email);
        var resetPassResult = await userManager.ResetPasswordAsync(user, request.Token, request.Password);
        if (!resetPassResult.Succeeded)
        {
            logger.LogError("ResetPassword failed for user {0}.", user.Id);
            var errors = resetPassResult.Errors
                .Select(error => Error.Validation("User." + error.Code, error.Description))
                .ToList();
            logger.LogInformation("Password reset failed for user: {0}", user.Id);
            return errors;
        }

        logger.LogInformation("Password reset successful for user: {0}", user.Id);
        return Result.Success;
    }
}