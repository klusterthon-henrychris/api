...
public class PaymentControllerI(IPaymentService paymentService) : BaseController
{
    [AllowAnonymous]
    [HttpGet("request-payment details")]
    public async Task<IActionResult> GetPaymentDetails(string invoiceId)
    {
        // for endpoints with allowAnonymous, consider generating a token before sending requests?
